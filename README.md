...
*   **물리적 분석 :** `Librosa` 라이브러리를 사용하여 BPM, Energy, Brightness, Rhythmic Intensity 수치를 계산합니다. 이는 음악의 "구조적 특징"을 파악합니다.
*   **의미적 분석 :** `LAION CLAP` 모델이 90초 분량의 오디오를 15초 단위로 잘라 분석하고, 이를 512차원 벡터로 변환하여 평균값을 냅니다. 이는 음악의 "분위기(Mood)"를 수치화합니다.
